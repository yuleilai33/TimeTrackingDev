        ...
        if ($bill) {
            return $all ? Excel::create($data['filename'], function ($excel) use ($data) {
                ...
                $excel->sheet('Hourly Eng.($' . number_format($data['bill'][0] - $nheng_total, 2) . ')', function ($sheet) use ($data) {
                    $sheet->freezeFirstRow()
                        ->row(1, ['Consultant', 'Engagement', 'Report Date', 'Position', 'Task', 'Billable Hours', 'Rate($)', 'Rate Type', 'Billed Type', 'Billed($)', 'Report Status','Description'])
                    ...
                    foreach ($data['hours'] as $i => $hour) {
                        ...
                        $eng = $arr->engagement;
                        if ($hour->rate_type == 0) {
                            $sheet->appendRow([$hour->consultant->fullname(), $eng->name, $hour->report_date, $arr->position->name, $hour->task->description, $hour->billable_hours, $hour->rate, $hour->rate_type == 0 ? 'Billing rate' : 'Pay rate', $eng->paying_cycle == 0 ? 'Hourly' : ($eng->paying_cycle == 1 ? 'Monthly' : 'Fixed'), $hour->billClient(), $hour->getStatus()[0],$hour->description]);
                        }
                    }
                });
